...
#resampling data
#if method1 == 'pad':
#    data = data.resample(sample_size, base = 0,loffset = sample_size).fillna(method = method1)
#elif method1 == 'mean':
#    data = data.resample(sample_size, base = 0,loffset = sample_size)
#    data = data.interpolate()
#    
data['im_disp'] = data['x'] - data['x'].shift()

#if method2 == 'pad':
#    data2 = data2.resample(sample_size, base = 0,loffset = sample_size).fillna(method = method2)
#elif method2 == 'mean':
#    data2 = data2.resample(sample_size, base = 0,loffset = sample_size)
...
plt.savefig('C:\Users\Win8\Documents\Dynaslope\Data Analysis\SVM Algorithm\\figs sensor\\'+site+' '+str(crack)+' '+' '+data.index[0].strftime('%Y-%m-%d %H-%M')+' to '+data.index[-1].strftime('%Y-%m-%d %H-%M') +' '+str(sample_size)+' '+method1+' '+method2+'.png',
